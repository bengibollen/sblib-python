...
- `python_reload` is core runtime functionality, not an efun module. It reloads the modules listed in `sblib.runtime.MODULES` and then re-runs registration, so keep that list aligned with the startup modules.
