...
- `sblib/runtime.py` is the central registry helper. It owns the list of active Python modules, wraps `ldmud.register_efun()` / `register_type()`, keeps an in-process registry for help/introspection, and implements explicit module reloads.
- `sblib/efuns/help.py`, `sblib/efuns/reload.py`, and `sblib/efuns/json.py` are the active efun modules loaded by startup.
...
- `python_reload` reloads the modules listed in `sblib.runtime.RELOADABLE_MODULES` and then re-runs registration, so keep those lists aligned with the startup modules.
